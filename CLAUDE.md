...
- Версии: **бэк 1.22.7, фронт 1.32.2**.
- **В работе — волна 03 «PRIME»** (тёмное стекло над лентой прожитых дней, DESIGN §10.2).
  Волна неактивна, смотреть через переключатель волн. Пер-тайл эталоны Playwright её
  **не покрывают**: блюр рендерится по-разному на GPU и в headless, канон остаётся за волной 01.
  На борд начали возвращаться виджеты — первым приехала музыка: чёрная плашка Spotify
  и там, где ничего не играет, очередь недавнего (обложка · трек · альбом · давность).
- **Фон волны — ОДИН слой с бордом**: `.wave-backdrop` растянут на `main` (`absolute`), а не
  прибит к вьюпорту, и едет вместе с плитками; заполняется удвоением текста, поэтому стена
  закрыта при любом зуме и любой длине прокрутки. Кадр экрана (виньетка PRIME) остаётся `fixed`
  сознательно — DESIGN §10.1/§10.2.
- **В мобильном СТЕКЕ высоту блоку не даёт никто — и это касается блоков ВОЛНЫ тоже** (DESIGN §8).
  Правило было про общие виджеты; телефон владельца показал, что скин ломается так же, только
  чинить обязан он: кадр Obscura (обложка — абсолютный квадрат) вырождался в полоску 358×102,
  ряд карт PRIME — в полоску высотой 20px. Оба получили свою форму под `.board-stack`: квадрат
  у кадра (только когда обложка ЕСТЬ) и 3:5 у карты волны — пропорция снята с борда, а не выдумана.
- **Соцсети в стеке — один ряд, а не квадрат 2×2** (DESIGN §8): квадрат разворачивался в
  контентную высоту двух рядов крупных спрайтов и съедал пол-экрана. Колонки едут переменными
  (`--social-cols` / `--social-count`), объявлены в CSS — inline-стиль стек не перебил бы.
- **`onerror` у картинки — НЕ полный признак «не загрузилась»**: недоступный CDN держит
  соединение открытым и ошибки не отдаёт никогда. Обложка Spotify ждёт восемь секунд, дальше
  считается неприехавшей и уступает место фирменной плашке (PRD §5.5, DESIGN §7.1). Пустое место
  вместо оговорённой заглушки — именно этот случай, а не поломка распознавания.
- **Хозяин переключателя волн обязан дать плитке ВЫСОТУ**, а не только ширину: карта волны
  PRIME растянута на плитку (`height: 100%`) и в шапке админки схлопывалась в ноль — кнопки
  были, но не нажимались. Админка задаёт 160×62, у карты есть пол высоты (DESIGN §2.6).
- **Жест наведения на плитке — параметр волны** (четыре токена на `.pixel-tile`, DESIGN §10.2).
  Волна со своим ховером **переопределяет токены**, а не гасит правило `:hover` волны 01;
  на стеклянной волне унаследованный `filter` подъёма гасил её же блюр.
- **Слои виджета музыки по умолчанию выключены** (подложка-обложка, знак Spotify, шкала головки,
  ссылка по кадру, обложка/альбом/давность в ряду недавнего) — включает волна. Это защищает
  эталоны волны 01 от чужих правок. Режим (`data-music-mode`) различает СОДЕРЖИМОЕ, но не корпус:
  скин берёт всю плитку, иначе виджет мигает материалом в секунду, когда трек дослушан.
- **Нет обложки — это состояние виджета, а не пустое место** (`data-music-art`). Вместо картинки
  встаёт знак Spotify на одной из пяти фирменных плашек (детерминированно по треку), а волна,
  у которой обложка несёт композицию, перестраивается: Obscura сжимает карточку по вертикали
  тем же приёмом, каким она уже жалась по горизонтали (`--music-card-h` рядом с `--music-card-w`).
- **Размеры карточки приезжают ПЕРЕМЕННЫМИ, а не inline-стилем** — обе оси. Inline скин не
  перебивает ничем, кроме `!important`, а обе оси волна вправе переопределить.
