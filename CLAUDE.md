...
- Версии: **бэк 1.16.1, фронт 1.23.2**.
- Последняя ветка: пустота ночи — ночь без сохранённых кусков показывает то же пустое
  состояние, что и день без сна (кровать + «нет данных о сне», общий `SleepNoData`).
- Перед этим: «ночь как она была» (I-23) — сырые куски сна теперь **хранятся**
