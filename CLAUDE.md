...
- Версии: **бэк 1.15.9, фронт 1.22.0**.
- Последняя ветка: листание прошлых недель в календаре (окно считается от опоры,
  граница месяца — пунктирная ступенька в жёлобе сетки).
- ⚠️ Открытый долг: Playwright-эталоны переснять в Docker после одобрения
  владельцем — рендер календаря изменился (служебный ряд навигации).
