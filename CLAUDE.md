...
- Версии: **бэк 1.16.1, фронт 1.23.3**.
- Последняя ветка: шапка «Сна» больше не подписывает выбранный день — эта подпись уже
  стоит в «Сегодня» и в календаре; проп `today` у тайла убран за ненадобностью.
- Перед этим: пустота ночи — ночь без сохранённых кусков показывает то же пустое
  состояние, что и день без сна (кровать + «нет данных о сне», общий `SleepNoData`).
- Ещё раньше: «ночь как она была» (I-23) — сырые куски сна теперь **хранятся**
  (`sleep_segment`), тайл «Сон» переключается `сумма ↔ по часам`, ночь разложена по
  дорожкам фаз (вертикаль = глубина сна). Сравнение с «обычной ночью» пробовали тремя
  способами и сняли целиком — причина в PRD §5.4. График появляется только для ночей,
  пришедших после этой ветки; прошлые добираются пересылкой широкого окна шорткатом.
- Что было раньше и почему так решили — `docs/journal.md`.
