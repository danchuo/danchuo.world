...
- `frontend/` — Next.js (App Router) + TypeScript + Tailwind v4. Публичный JSON-клиент к Quarkus (только чтение).
  ...
  - **B2 (фронт):** `AnalyticsBeacon` расширен сбором кликов — глобальный capture-listener ищет ближайший `[data-tile-id]`, считает долю точки внутри его `getBoundingClientRect` (0..1), копит в буфер (потолок 50) и шлёт **батчем на уходе** (`postInteractions` → `sendBeacon`). `Board` навесил `data-tile-id` на обёртки тайлов (bento + стек) — рендер не изменён. Приватный просмотр — `app/admin/heatmap/page.tsx` (под noindex-layout `/admin`, токен из того же `sessionStorage`): рисует bento по `resolveLayout(null)`, заливая тайлы `color-mix` по доле кликов + таблица; админ-клиент `getHeatmap`. Ссылка из `/admin`. По умолчанию окно — **последние 7 дней** (дефолт в `AnalyticsSummaryResource`); `path` зашит в `/` (поле не выносим — публичная страница одна, `path`/период остаются в API). Бикон **не трекает `/admin*`** (клики владельца не мусорят аналитику).
...
This project is indexed by GitNexus as **danchuo.world** (2057 symbols, 4173 relationships, 169 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
