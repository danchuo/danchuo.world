...
- `backend/` — Quarkus (Kotlin), Gradle. Feature-пакеты (вертикальные слайсы, PRD §3.1) + `core` (сквозные соглашения).
  ...
  - `core` содержит bearer-фильтр (`/api/ingest/*`), мягкий рейтлимит публичных GET (`RateLimitFilter`, in-memory токен-бакет по IP; `/api/film-media` исключён), MSK/генезис-конфиг и кэш-шов.
