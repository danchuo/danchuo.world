...
- Версии: **бэк 1.18.1, фронт 1.25.0**.
...
- **Задеплоенный Caddyfile сам собой не применяется.** Он подключён bind-маунтом, а
  определение сервиса `caddy` при этом не меняется — `docker compose up -d` оставляет
  контейнер как есть, и Caddy продолжает отдавать конфиг, разобранный на старте. Новый
  маршрут выкатывается зелёным и отвечает 404 до первого рестарта. Деплой теперь зовёт
  `caddy reload` явно (zero-downtime, с валидацией: битый конфиг падает в CI, а не на бою).
