...
| `bike` | Велобайк: поездки, покупки тарифов, геокодинг станций (Nominatim); **свой веломаршрутизатор** — граф OSM ресурсом (`bike/moscow-bike-graph.bin.gz`, собирается `backend/tools/build-bike-graph.py`; у дороги есть класс, и метр по проспекту «дороже» метра по велодорожке) + A\*, на нём кнопка «случайный путь» и петли для поездок, вернувшихся на ту же станцию | `GET /api/rides`, `/api/rides/stats`, `/api/rides/{id}/random-paths` |
...
This project is indexed by GitNexus as **danchuo.world** (7052 symbols, 15270 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
