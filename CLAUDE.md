...
- Версии: **бэк 1.15.9, фронт 1.22.1**.
- Последняя ветка: Playwright-эталоны пересняты в Docker (`update` теперь означает
  `--update-snapshots=all`); открытых долгов по визуальной регрессии нет.
