 ...
 * `0090-analytics` and `1080-analytics-dimensions`: Postgres does not truncate an over-long
 * value, it fails the insert — and these endpoints take their input from anybody. PRD §5.11
 */
internal object AnalyticsLimits {
    const val PATH = 512
    const val VISIT_ID = 64
    const val REFERRER = 512
    const val REFERRER_HOST = 255
    const val UTM = 128
