...
/**
 * Один придуманный путь для кнопки «нарисовать случайный путь» (PRD §9 B4). Точки — компактными
 * парами `[lat, lon]`, а не объектами: ломаная роутера это сотни точек, и имена полей на каждой
 * из них удвоили бы ответ.
 *
 * Путь **нигде не хранится**: пачка живёт ровно до закрытия окна. Поэтому у него нет и id.
 */
data class RandomPathView(
    val points: List<List<Double>>,
    /** Длина придуманного пути в метрах — по версии роутера. */
    val distanceMeters: Int,
    /** Кратчайший веломаршрут между теми же станциями: знаменатель коридора. */
    val optimumMeters: Int,
    /** На сколько процентов путь длиннее оптимума. Всегда в [0, 40] — это и есть коридор. */
    val overPercent: Int,
)
