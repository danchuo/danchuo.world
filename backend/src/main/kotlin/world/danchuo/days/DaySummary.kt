...
data class DaySummary(
     ...
     *
     * Свёртки [disciplineDone] для линзы календаря мало: остановка карты закрывается порогом
     * `count >= occurrence` (у пунктов с target=2 остановок две и они разные), а «пункт закрыт
     * целиком» — это только последняя из них. Нули присутствуют намеренно: так «пункт был, но не
     * сделан» отличимо от «пункта в этот период не существовало».
     */
