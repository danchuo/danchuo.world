...
data class DisciplineItemView(
     ...
     * пунктов, кроме `podcasts`, и у него же пусто, пока ни один эпизод не набрал порог.
     *
     * Длина НЕ обязана совпадать с [count]: отметки считаются по сумме минут за сутки, а карточки
     * поэпизодно. Двухчасовой эпизод закрывает обе остановки и даёт одну карточку — это норма,
     * фронт раздаёт карточки по порядку и оставляет лишние остановки без ховера.
     */
    val episodes: List<PodcastEpisodeView>,
)

/**
 * Карточка прослушанного эпизода (§5.6) — то, что показывается при наведении на остановку пункта
...
data class PodcastEpisodeView(
    ...
    val showUrl: String?,
    val imageUrl: String?,
    /** Сколько реально слушали, минут. */
    val listenedMinutes: Int,
    /** Полная длительность эпизода, минут; `null` — не приехала. Для строки «47 из 48». */
    val durationMinutes: Int?,
)
