...
- **Правка Caddyfile тоже раскатывается сама** — но только потому, что деплой после
  `up -d` явно зовёт `caddy reload`. Одного `up -d` тут мало: Caddyfile подключён
  bind-маунтом, определение сервиса не меняется, контейнер не пересоздаётся, и Caddy
  держит конфиг, разобранный на старте (на этом уже потеряли раскатку маршрута `/dav`).
  Reload валидирует конфиг перед применением, поэтому битый Caddyfile валит деплой,
  а не боевой edge. Следом деплой мягко проверяет, что `/dav` отвечает 401 —
  это признак, что запрос дошёл до WebDAV, а не до Next.
