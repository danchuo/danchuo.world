 ...
 * которой в ней уже нет. Уезжает вся ячейка целиком. На закрытии она
...
export function useDropMorph({
   ...
   * прячутся на время его полёта и возвращаются вместе с ним.
   */
  ...
  const playIn = useCallback(() => {
    ...
    const p = parts();
    // Мерить пока нечего — пробуем ещё раз на следующем кадре. Повтор нужен не для точности
    // замера, а потому, что мерить бывает НЕЧЕГО: галерея ленты дропов открывается после
    // доводки ленты в середину и грузит кадры после клика, так что первая попытка приходится
    // на незаконченную прокрутку или на сцену без кадра вовсе. Сдайся шов с первой неудачи —
    // и проявка играла бы только у верхнего дропа, которому доводка не нужна.
    //
    // Замер принимается СРАЗУ, как только он годен: ждать «устоявшегося» размера рассмотрено
    // и отклонено (DESIGN §7.5). Попытки ограничены: у мозаики героя нет вообще, и молчаливый
    // бесконечный цикл там был бы хуже отсутствия движения.
