...
export interface DisciplineItemView {
   ...
   * у `podcasts`, и только для эпизодов, набравших порог.
   *
   * Длина НЕ обязана совпадать с `count`: отметки считаются по сумме минут за сутки, а
   * карточки поэпизодно. Двухчасовой эпизод закрывает обе остановки и даёт одну карточку —
   * карта раздаёт карточки по порядку, лишняя остановка остаётся без ховера.
   */
  episodes?: PodcastEpisodeView[];
}

/** Карточка прослушанного эпизода (§5.6): обложка, эпизод и шоу со ссылками, сколько слушали. */
