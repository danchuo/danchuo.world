...
export interface ReadingBookView {
  ...
  /** Id захода — ключ к его пересказу (`GET /api/reading/summary/{id}`). */
...
/**
 * Пересказ прочитанного за заход куска (§5.16): пункты и строка-итог.
 *
 * Собран **по тексту самой книги** — читалка синкает epub вместе со статистикой, и бэкенд
 * вырезает из него ровно тот кусок, что стоит на карточке («48% → 53%»). Пересказа «по памяти
 * модели» здесь не бывает: нет файла книги — нет и кнопки.
 */
export interface ReadingSummaryView {
  bullets: string[];
...
export interface PodcastEpisodeView {
  ...
  /** ISO-момент начала захода; в подпись идёт временем MSK (`09:12`). */
  startedAt: string;
  /** Сколько слушали В ЭТОТ ЗАХОД, минут; эта же цифра стоит под своей остановкой. */
  listenedMinutes: number;
  /** Сколько прошли по этому эпизоду за все заходы дня, минут; равно `listenedMinutes` у одиночного. */
  dayMinutes: number;
...
export interface DayView {
  ...
  /**
   * Отмечали ли монстра за этот день (§5.6). Без него `monster === null` двусмысленно: так
   * выглядит и честное «не пил», и день, за который интерактивный шорткат не запускали.
   * По `hasData` это не различить — запись дня создаёт health-ingest (авто 12/18/24 MSK).
   *
   * Опционально: ответы старого кэша поля не несут. Отсутствие читаем как `false` («не
   * отмечали») — молчать безопаснее, чем утверждать чистый день, которого могло не быть.
   */
  monsterReported?: boolean;
  /**
...
export interface DaySummary {
  ...
  /**
   * Отмечали ли монстра за день (§5.6) — тот же признак, что в [DayView]. Линзе календаря
   * он нужен, чтобы день без запуска шортката не попадал в «не пил» наравне с честно чистым.
   * Опционально: старый кэш поля не несёт ⇒ читается как «не отмечали».
   */
  monsterReported?: boolean;
}
