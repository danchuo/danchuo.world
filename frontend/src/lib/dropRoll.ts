...
export const FRAME_WHEEL_TRAVEL_PX = 80;
/**
 * Сколько кадр стоит на месте после шага, прежде чем плитка примет следующий (мс).
 * Совпадает с длиной въезда кадра: быстрее — и кадры сменяли бы друг друга, не успев встать.
 */
export const FRAME_STEP_COOLDOWN_MS = 260;
 ...
 * идут ровной чередой, по одному за остывание. Хвост инерции после короткого маха при этом
 * почти ничего не добавляет: пока шаг остывает, путь **не копится вовсе** (возвращаем `acc: 0`),
 * а к концу остывания инерция уже выдохлась и до порога не дотягивает.
 *
